...
local SELECT = 'SELECT id, owner, plate, vehicle, model, garage, state, depotprice FROM owned_vehicles'

---@param r table?
---@return table?
local function normalize(r)
    if not r then return nil end
    local props = r.vehicle
    ...
    return {
        id = r.id,
        citizenid = r.owner,
        modelName = r.model or props.model,
        props = props,
        garage = r.garage,
        state = r.state,
...
function V.GetPlayerVehicles(filter)
    filter = filter or {}
    local where, params = {}, {}
    if filter.citizenid then where[#where + 1] = 'owner = ?'; params[#params + 1] = filter.citizenid end
...
function V.GetVehicleIdByPlate(plate)
    ...
    return MySQL.scalar.await('SELECT id FROM owned_vehicles WHERE plate = ?', { plate })
end

--- @param entity number  (statebag `vehicleid` lo pone el garaje tras spawnear)
...
function V.SaveVehicle(entity, data)
    ...
    if data.props ~= nil then
        sets[#sets + 1] = 'vehicle = ?'; params[#params + 1] = json.encode(data.props)
    ...
    params[#params + 1] = id
    MySQL.update.await(('UPDATE owned_vehicles SET %s WHERE id = ?'):format(table.concat(sets, ', ')), params)
    return true
end

--- @param data { model: string, citizenid: string, garage?: string, props?: table, state?: integer, type?: string }
--- @return integer? id
function V.CreatePlayerVehicle(data)
    ...
    if not plate then return nil end
    if props.model == nil then props.model = GetHashKey(data.model) end

    return MySQL.insert.await(
        'INSERT INTO owned_vehicles (owner, plate, vehicle, type, model, garage, state) VALUES (?, ?, ?, ?, ?, ?, ?)',
        {
            data.citizenid,
            plate,
            json.encode(props),
...
function V.SetPlayerVehicleOwner(id, citizenid)
    ...
    local n = MySQL.update.await('UPDATE owned_vehicles SET owner = ? WHERE id = ?', { citizenid, id })
    return (n or 0) > 0
end
...
function V.ClaimOwner(id, citizenid, requireState)
    ...
    local n = MySQL.update.await(
        'UPDATE owned_vehicles SET owner = ? WHERE id = ? AND state = ?', { citizenid, id, requireState })
    return (n or 0) > 0
end

--- @param field 'vehicleId'|'plate'|'citizenid'
function V.DeletePlayerVehicles(field, value)
    local col = field == 'vehicleId' and 'id' or field == 'plate' and 'plate' or field == 'citizenid' and 'owner'
    if not col then return false end
    local n = MySQL.update.await(('DELETE FROM owned_vehicles WHERE %s = ?'):format(col), { value })
    return (n or 0) > 0
end

function V.SetState(id, state)
    MySQL.update.await('UPDATE owned_vehicles SET state = ? WHERE id = ?', { state, id })
end

function V.ClaimState(id, fromState, toState)
    local n = MySQL.update.await('UPDATE owned_vehicles SET state = ? WHERE id = ? AND state = ?', { toState, id, fromState })
    return (n or 0) > 0
end

function V.SetStateGarage(id, state, garage)
    MySQL.update.await('UPDATE owned_vehicles SET state = ?, garage = ? WHERE id = ?', { state, garage, id })
end

function V.ClaimStateGarage(id, fromState, toState, garage)
    local n = MySQL.update.await('UPDATE owned_vehicles SET state = ?, garage = ? WHERE id = ? AND state = ?', { toState, garage, id, fromState })
    return (n or 0) > 0
end

function V.SetGarage(id, garage)
    MySQL.update.await('UPDATE owned_vehicles SET garage = ? WHERE id = ?', { garage, id })
end

function V.ClaimGarage(id, fromGarage, toGarage, requireState)
    local n
    if fromGarage == nil then
        n = MySQL.update.await('UPDATE owned_vehicles SET garage = ? WHERE id = ? AND garage IS NULL AND state = ?', { toGarage, id, requireState })
    else
        n = MySQL.update.await('UPDATE owned_vehicles SET garage = ? WHERE id = ? AND garage = ? AND state = ?', { toGarage, id, fromGarage, requireState })
    end
    return (n or 0) > 0
end

function V.SetStateDepotPrice(id, state, depotPrice)
    MySQL.update.await('UPDATE owned_vehicles SET state = ?, depotprice = ? WHERE id = ?', { state, depotPrice, id })
end

--- @return table<string, integer>
function V.CountByGarage()
    local rows = MySQL.query.await('SELECT garage, COUNT(*) AS n FROM owned_vehicles WHERE garage IS NOT NULL GROUP BY garage') or {}
    local out = {}
...
function V.MileageFor(ids)
    ...
    local ok, rows = pcall(MySQL.query.await, ('SELECT id, mileage FROM owned_vehicles WHERE id IN (%s)'):format(ph), ids)
...
function V.AdminList(opts)
    ...
    if type(opts.search) == 'string' and opts.search ~= '' then
        ...
        where = ' WHERE plate LIKE ? OR model LIKE ? OR owner LIKE ?'
        params = { like, like, like }
    end

    local total = MySQL.scalar.await('SELECT COUNT(*) FROM owned_vehicles' .. where, params) or 0
